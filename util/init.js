...
init.prototype.Image = function(src) {
	let image = new Image()
	image.src = src
	image.attribute = {}
...
init.prototype.checkSpriteMargin = function(mainSprite, otherSprite) {
	...
	log("mainSprite:", mainSprite.attribute)
	log("otherSprite:", otherSprite)
	...
	log("mainSpritePosition:", mainSpritePosition)
	...
	log("checkOtherSprite:", checkOtherSprite)
	...
	for (let i in checkOtherSprite) {
		...
		let centerX = Math.abs(Math.abs(mainSpritePosition["center"][0]) - Math.abs(checkOtherSprite[i]["center"][0]))
		let centerY = Math.abs(Math.abs(mainSpritePosition["center"][1]) - Math.abs(checkOtherSprite[i]["center"][1]))



		if ((centerX <= checkOtherSprite[i].width/2 + mainSpritePosition.width/2) && (centerY <= checkOtherSprite[i].height/2 + mainSpritePosition.height/2)) {
			logs.error("发生了碰撞")
			return false
		}
	}
